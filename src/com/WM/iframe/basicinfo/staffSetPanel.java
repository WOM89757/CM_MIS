//Source file: C:\\Users\\茂\\Desktop\\课设\\管理信息系统\\MIS\\CMMIS\\src\\com\\WM\\iframe\\basicinfo\\staffSetPanel.java
...
public class staffSetPanel extends JPanel 
{
   ...
   /**
   存储列标题的数组
   @roseuid 5D0B97BE011D
    */
   public staffSetPanel() 
   {// 设置经手人面板
		...
		columnNames = new String[] { "编号", "姓名", "性别",  "联系电话", "禁用" };// 存储列标题的数组
		...
		button.addActionListener(new ActionListener() {// “删除”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				int op = JOptionPane.showConfirmDialog(staffSetPanel.this, "确认要删除该操作员？");// 弹出“确认要删除该操作员？”的提示框
		...
		enableButton.addActionListener(new ActionListener() {// “禁用/启用”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				...
				if (selRow < 0)// 选定行的索引小于0
					return;// 退出应用程序
				int op = JOptionPane.showConfirmDialog(staffSetPanel.this, "确认要修改该操作员状态？");// 弹出“确认要修改该操作员状态？”的提示框
				if (op == JOptionPane.OK_OPTION) {// 点击“确认”
					if (selRow < 0)// 选定行的索引小于0
						return;// 退出应用程序
					String id = table.getValueAt(selRow, 0).toString().trim();// 获得经手人编号
