//Source file: C:\\Users\\茂\\Desktop\\课设\\管理信息系统\\MIS\\CMMIS\\src\\com\\WM\\iframe\\infoQuery\\sellQuery.java
...
public class sellQuery extends JInternalFrame 
{
   ...
   {// 销售查询内部窗体的构造方法
		...
		showAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				content.setText("");
				List list=DAO.findForList("select * from view_billinfoSearch");
		...
		String[] tableHeads = new String[]{"销售票号", "商品名称", "商品编号", "单价",
				"数量", "金额", "客户", "销售日期", "员工"};
   ...
   private final class QueryAction implements ActionListener 
   {
      ...
      public void actionPerformed(final ActionEvent e) 
      {
			...
			List list=null;// 结果集
			String con = condition.getSelectedIndex() == 0 ? "userName " : "billId ";
			int oper = operation.getSelectedIndex();
			String opstr = oper == 0 ? "= " : "like ";
			String cont = content.getText();
			list = DAO.findForList("select * from view_billinfoSearch where "
					+ con
					+ opstr
					+ (oper == 0 ? "'"+cont+"'" : "'%" + cont + "%'")
					+ (selDate ? " and date>'" + startDate.getText()
							+ "' and date<='" + endDate.getText()+" 23:59:59'" : ""));// 执行拼接的SQL语句后获得的结果集
